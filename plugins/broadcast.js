   ...
   async: async (m, {
      ...
      try {
         ...
         if (/image\/(webp)/.test(mime)) {
            ...
            return client.reply(m.chat, Func.texted('bold', `🚩 Successfully send broadcast message to ${id.length} ${command == 'bc' ? 'chats' : command === 'bcprem' ? 'premium users' : 'groups'}`), m)
         }
         ...
         client.reply(m.chat, Func.texted('bold', `🚩 Use this command with text or by replying to an image, video or audio.`), m)
      } catch (e) {
